...
| **fleet-ops** | Companion to the `orchestrating-agent-fleets` skill. `/como-vamos [wave]` (alias `/fleet-status`) — per-pod progress, in-flight work per agent, handoff readiness (silent-failure guard), Test Pod evidence, needs-attention list. `/equipo [wave]` (alias `/fleet-graph`) — team org chart + dependency graph, with live agent state from `herdr agent list`. `/switch-backup` — failover the session to its role's backup model (config `.pi/fleet-models.json`; role via `--fleet-role`/`FLEET_ROLE`; roles: master·monitor·architect·worker·reviewer·test-worker); also auto-offers on 429/503. `/watchdog` + a deterministic monitor-only timer (`watchdogSeconds`, default 60) that surfaces overdue `NEXT_CHECK_IN` entries AND per-agent context pressure (reads each agent's session tokens via `herdr agent list`; `contextWarnPercent`/`contextHighPercent`, default 70/85) — all without involving the master |
